...
The material builder derives the major-activity target from trip length and pace, separates nearby meal recommendations from activity counts, groups activities geographically by day, and keeps one primary stay. Product requests query only the proposed daily route chains with concurrency capped at four; legacy fixtures retain their directed-matrix contract. Typed per-edge failures remain visible, while exact-cent budget targets use versioned weights. The committed fixture report records 5/5 cases and 42/42 expected legacy route edges. The allocator creates planning targets rather than price facts; its bundle is consumed by the Plan Agent described below rather than being presented as an itinerary itself.
...
The Plan Agent makes one schema-constrained placement call for ready or recoverable partial material bundles that still contain Provider-grounded POIs. Deterministic code preserves candidate facts, sources, available route edges, weather risks, complete dates, IDs, and validation lineage; missing routes remain null and explicit instead of being model-filled. The current fixture report records 6/6 cases, 13/13 grounded scheduled candidates, five planned cases, and one zero-model-call blocked stop. The older DeepSeek point-in-time report predates this recovery contract and is not combined with the refreshed fixture counts. Budget allocations remain targets rather than verified prices, so the Plan Agent emits no fabricated `CostItem`. Its isolated baseline does not evaluate finalization rules; the downstream Hard Validator does.
