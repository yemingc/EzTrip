...
必须把每个 allowed_poi_candidate_id 恰好安排一次, 不得新增、改名、遗漏或安排酒店 ID。
...
def _activity_kind(candidate: CandidatePOI) -> ActivityKind:
    dining_markers = ("餐", "美食", "小吃", "restaurant", "food", "cafe", "café")
    searchable = " ".join((*candidate.categories, *candidate.tags)).casefold()
    if any(marker in searchable for marker in dining_markers):
        return ActivityKind.MEAL
    return ActivityKind.ATTRACTION
...
def normalize_plan_response(
    ...
    valid_days = {item.day_number: item.date for item in materials.planner_context.days}
    placements: defaultdict[date, list[tuple[PlannerPlacementProposal, CandidatePOI, datetime]]] = (
        defaultdict(list)
    )
    for proposal in response.proposal.items:
        day = valid_days.get(proposal.day_number)
        if day is None:
            raise PlanAgentProtocolError("Plan Agent proposal references a day outside the trip")
        ...
        placements[day].append((proposal, candidate, starts_at))
    ...
    for context_day in materials.planner_context.days:
        ...
        for proposal, candidate, starts_at in sorted(
            placements.get(day, []), key=lambda item: item[2]
        ):
        ...
        try:
            day_plan = DayPlan(date=day, items=items, weather_risk_ids=risk_ids)
        except ValidationError as error:
            raise PlanAgentProtocolError(
