...
from app.agents import run_live_single_planner
from app.core.config import get_settings
from app.evaluation import evaluate_single_planner_suite

REPOSITORY_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_OUTPUT_PATH = (
    REPOSITORY_ROOT / "evals" / "reports" / "deepseek-single-planner-baseline-2026-08-21.json"
)


def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the 10-case single-Planner evaluation against live DeepSeek."
    )
    parser.add_argument(
        "--live",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = asyncio.run(
        evaluate_single_planner_suite(
            lambda context, candidates: run_live_single_planner(
                context,
                candidates,
                settings,
            ),
        ...
        json.dumps(
            {
                ...
                "candidate_coverage_rate": str(report.candidate_coverage_rate),
