...
from app.agents import run_live_stay_agent
from app.core.config import get_settings
from app.evaluation import evaluate_stay_agent_suite

REPOSITORY_ROOT = Path(__file__).resolve().parents[2]
DEFAULT_OUTPUT_PATH = (
    REPOSITORY_ROOT / "evals" / "reports" / "deepseek-stay-agent-baseline-2026-08-21.json"
)
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run four eligible Stay cases against live DeepSeek; two blocked cases skip calls."
        )
    )
    parser.add_argument(
        "--live",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    report = asyncio.run(
        evaluate_stay_agent_suite(
            lambda context, provider: run_live_stay_agent(context, provider, settings),
            execution_mode="live",
        ...
        json.dumps(
            {
                ...
                "context_reference_coverage_rate": str(report.context_reference_coverage_rate),
                ...
                "commercial_truth_boundary_passed": (report.commercial_truth_boundary_passed),
