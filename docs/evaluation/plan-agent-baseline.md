...
这些指标不是行程“准确率”、实时高德质量、酒店可订性、预算满足率或生产 SLA。候选目录属于开发 fixture，本隔离套件不评估营业时间、must/avoid、路线时间窗等定稿规则；这些规则现由 EZ-303 Hard Validator 的独立 fixture suite 覆盖，但 Repair Router 仍未实现。因此简历中应描述为“版本化 grounding/lineage regression”，不能写成“真实用户行程成功率 100%”。
