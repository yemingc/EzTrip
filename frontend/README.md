...
真实浏览器 canary 单独运行且默认跳过；显式启用会消耗 DeepSeek 与高德配额，并接受“已保存 PlanVersion”或“带稳定错误码的诚实降级”两种安全终态：
